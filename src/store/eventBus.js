...
export const eventBus =  new Vue({
    ...
    methods: {
       ...
       * @param {*} message 
       */
      chtListSrch(message,newchat,date) {
        this.$emit('chtListSrch', message,newchat,date);
      } ,
      ...
      setManagerInfo(managerInfo) {
        this.$emit('setUserInfo',managerInfo);
      },
      /**
      ...
      setCuslInfo(cuslInfo) {
        this.$emit('setUserInfo',cuslInfo);
      },

      /**
       * 조직정보, 조직트리 컴포넌트 확인 버튼 이벤트
       * @param {*} userInfo
       */
      setOgnzTreeInfo(ognzInfo) {
        this.$emit('setOgnzTreeInfo',ognzInfo);
      },
       ...
       */
      initFindMng() {
        this.$emit('initFindMng');
      },
      /**
       * 상담원 검색 이벤트
       */
      findCusl(cuslName) {
        this.$emit('findCusl', cuslName);
      },
       ...
       * @param {*}  USER_CHT_STAT
       */
      USER_CHT_STAT(USER_CHT_STAT){
        this.$emit('USER_CHT_STAT', USER_CHT_STAT);
      },
       ...
       * 채팅 내역 조회
       */
      chatHstSrch(chat, clickChat){
        this.$emit('chatHstSrch', chat, clickChat);
      },
       ...
       * 채팅에서 고객정보 조회
       */
      searchCustInfo(phnNo, chtUserKey, chnl,chtCuttId){
        this.$emit('searchCustInfo', phnNo, chtUserKey, chnl, chtCuttId);
      },

      /**
       * 새상담 만들기기 바로처리일 경우
       */
      searchCustInfoNewCutt_4(chtUserKey,chnl,chtCuttId){
        this.$emit('searchCustInfoNewCutt_4', chtUserKey, chnl, chtCuttId);
      },

      /**
       * 바로처리가 아닐경우 새상담 만들기기
       */
      searchCustInfoNewCutt(chtUserKey,chnl,chtCuttId){
        this.$emit('searchCustInfoNewCutt', chtUserKey, chnl, chtCuttId);
      },

      /**
       * 바로처리가 아닐경우 새상담 만들기
       */
      newCuttOnCsInfoTable(chtUserKey,chnl,chtCuttId){
        this.$emit('newCuttOnCsInfoTable', chtUserKey, chnl, chtCuttId);
      },

      //임대주택관리현황 윈도우 팝업 오픈
      openRightRentalManage(item){
        this.$emit('openRightRentalManage',item);
      },
       ...
       * 지식 컨텐츠 슬라이드
       */
      openKnowledgeSlide(contentId){
        this.$emit('openKnowledgeSlide',contentId);
      },
    },
