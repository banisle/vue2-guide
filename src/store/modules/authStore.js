...
const authStore = {
  ...
  getters: {
    ...
    GE_AUTH_LOGIN_IC_CTIQ: (state) => state.login_data.IC_CTIQ,      // @njy SMS 인증용 임시 데이터, SMS인증이 성공하면 저장된 login_temp_data로 로그인 완료 처리 후 사용자 정보를 세팅함. 인증이 끝나면 삭제
  },
  mutations: {
    ...
    MU_AUTH_LOGIN_IC_CTIQ: (state, data) => {
      state.login_data.IC_CTIQ = data;
    },
  },
  actions: {
    //로그인
    ...
    async AC_AUTH_LOGIN_DLGT({ commit }, data) {
      // 로그인(권한위임)
      let set_data = {
        code: 0,
        data: data,
      ...
      await api
          ...
          .then((response) => {
            ...
            window.sessionStorage.setItem("token", response.data.HEADER.DLGT_ACCESS_TOKEN );
