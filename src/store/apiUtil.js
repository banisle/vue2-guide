...
api.interceptors.request.use(
  async (request) => {
    ...
    if(request.url === '/auth-api/login' || request.url === '/api/palette-main/menu/list'){
      request.headers['CERT-CUSTCO-ID'] = ''; // 로그인 시에는 센터 코드를 비워준다.
    }
...
api.interceptors.response.use(
  async (response) => {
    ...
    if( response.data.HEADER != undefined ) {
      // header.token 자동 갱신
      if( response.data.HEADER.DLGT_ACCESS_TOKEN ) {
        window.sessionStorage.setItem("token", response.data.HEADER.DLGT_ACCESS_TOKEN); //스텔스모드의 경우(위임등)
      }else {
