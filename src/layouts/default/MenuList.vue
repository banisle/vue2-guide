<template>
  <v-list-item
    ...
    color="white"
    class="transparent"
    @click="item.ADMIN_ADD_MENU?item.SETTING_MENU?[$eventBus.$emit('showSidePanelCall', {}), dialogSetting = !dialogSetting]:[$store.dispatch('commonStore/AC_COMMON_SETTING_SLIDE', false),$eventBus.$emit('showSidePanelCall', item)]:mixin_set_active_tab(item)"
  >
    <!-- {{ item }} -->
    <v-list-item-content>
      <v-list-item-title
