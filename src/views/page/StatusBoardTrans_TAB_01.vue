<template>
  <div>
    ...
    <div class="d-flex align-center is-mt-s px-5">
      <span class="pl-label">이첩요청 (<span class="is-txt-blue">{{ TRANS_TOT_CNT }}</span>)건</span>
      <compo-tooltip-btn
        TitleProp="새로고침"
    ...
    <div
      ...
      <div
        ...
        <div
          ...
          <div class="pl-rounded-box pa-2 ml-12 ">
            {{ item.REQ_CNTNT }}
          </div>
        </div>
      </div>
...
<script>
export default {
  name: 'StatusBoardTrans_TAB_01', //이첩 요청
  props:{
    callEvt: {
      type: Boolean,
      default: false,
  ...
  methods: {
    async trnsfStatList(sState) {
      let postParam = {
        REQ_YN : 'Y' //요청 여부(요청과 접수가 동일 코드 이기 때문에)
        , TRANS_TYPE : 'ISMATRDV001' //이첩요청
      };
