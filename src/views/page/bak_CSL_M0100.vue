...
<div
    ...
    :style="`grid-template-columns: ${ cusArea_resizeClass === true ? first_drag +'fr 75fr' : first_drag+'fr 38fr 39fr'}`">
    <div
      class="pl-mainchat-csinfo is-col-fix pl-card-wrap"
      ...
      <mainphone-mydesk
        ...
        class="pl-mainphone-mydesk pl-mainphone-key-info "
        @toggle="toggleResize()"
      />
    ...
    <div
      ...
      >
      <div class="pl-card-top is-border-bottom" style="flex-wrap: wrap;" :class="mixin_getCustcoSetting('ENV_TMPL_TP') !== 'DEFAULT' ? 'pb-0' : ''">
        <div>
        ...
        <div class="pl-btn-wrap ml-auto align-self-start">
          ...
          <!-- 영상통화  -->
          <compo-tooltip-btn
            v-if="mixin_getCustcoSrvcStat('VIDEO') && CUST_INFO.CUST_ID != undefined && CUST_INFO.CUST_ID > -1 && !this.CALL_ING"
      ...
      <v-tabs-items
        ...
        class="pl-tabs-items"
        :style="mixin_getCustcoSetting('ENV_TMPL_TP') === 'DEFAULT' ? !demoVideoMode ?  'height: calc(100vh - 178px);' : 'height: 71px;' : 'height: calc(100vh - 214px)'">
        <v-tab-item>
          <!-- 상담내용 -->
          <!-- stt component -->
    ...
    <div
      ...
      <mainchat-all-log
        ...
        class="pl-mainchat-csinfo-history "
      />
    </div>
    <!-- dialog -->
    <!-- <v-dialog
...
import { eventBus } from "@/store/eventBus.js";
  ...
  computed: {
    ...
    computedCsHeight() {
      ...
      if( this.mixin_getCustcoSetting('ENV_TMPL_TP') !== 'DEFAULT' ) {
        ...
        calHeight = stt_switch ? 'calc(100vh - 655px)' : 'calc(100vh - 248px)'
      }
      return {
        height: calHeight
      }
  ...
  async created() {
    ...
    //   console.log('this.VST_RSVT_INFO : ', VST_RSVT_INFO);
    //   this.VST_RSVT_INFO = VST_RSVT_INFO;
    //   this.mixin_showDialog('AssignManager');
    //   setTimeout(() => this.$eventBus.$emit('initRsvt'), 100);
    // });
            ...
            if(phnNoConfirm){
              if(this.mixin_isEmpty(this.$store.getters['alertStore/GE_ALERT'].custPhnNo)){
                ...
                if(this.$store.getters['alertStore/GE_ALERT'].custPhnNo.replace(/[^0-9]/g, '').length < 11){
                  ...
                  this.iframeUrl = process.env.VUE_APP_VIDEO_URL + '/view/ssologin?loginId='+this.$store.getters['userStore/GE_USER_ROLE'].lgnId.replace('#', '%23')+'&tenantName=hkcloud&site='+this.$store.getters['userStore/GE_USER_ROLE'].company.SCHEMA_ID+'_'+this.$store.getters['userStore/GE_USER_ROLE'].company.CD+'&callid='+callId;
                  this.$store.commit("alertStore/hideAlert");
                }
              }
            }else{
    ...
    receiveMsgFromChild( e ) {
      ...
      if(this.demoVideoMode){
        if(message.key === "Invite") { //고객 초대 SMS URL
          ...
          data.sndngSeCd = 'LMS';
          data.message = sMessage + message.val; //고객 초대 SMS URL
