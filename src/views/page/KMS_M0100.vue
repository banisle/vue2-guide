      ...
      <div class="pl-cols flex-grow-1">
        ...
        <div class="is-col-fix is-vrt" style="width: 500px">
          ...
          <div class="pl-search-header pl-card-body py-3">
            <div class="is-left">
              <label class="font-weight-bold">지식 분류</label>
            </div>
          ...
          <div class="pl-card-body pl-tree-body is-border-top" style="height: calc(-345px + 100vh)">
            <v-treeview
              ref="treeDefault"
              activatable
              return-object
        ...
        <!-- right -->
        <div class="is-vrt d-flex flex-column fill-height">
          <div class="pl-card ">
          ...
          <div class="pl-card is-noborder flex-grow-1">
            <v-form ref="form">
                ...
                <div class="pl-form-inline-wrap vertical label-80">
                  ...
                  <div class="pl-form-inline">
                    <span class="pl-label">
                      ...
                      <!-- 툴팁 버튼 -->
                      <compo-tooltip-btn
                        TitleProp="선택한 지식분류에 관리부서를 지정하실 경우<br>우측의 돋보기 아이콘을 클릭하신 후<br>지정할 부서를 선택하십시오.<br><br>부서가 지정되면 해당 부서만 컨텐츠를 등록/수정할 수 있습니다."
                        ClassProp="pl-tooltip-btn"
                        IconProp="pl-icon20 question"
                        TooltipPositionProp="bottom"
                      ></compo-tooltip-btn>
                    ...
                    <div class="pl-desc">
                      ...
                      <v-text-field
                        class="pl-form type-middle is-lg flex-grow-0"
                        value="관리 담당부서 선택"
                        v-model="detailInfo.MNG_DEPT_NM"
                        disabled
                      >
                        <template v-slot:append>
                          <v-btn
                            v-if=" detailInfo.MNG_DEPT_ID !== '' && detailInfo.MNG_DEPT_NM != '' "
                          ...
                          <v-btn
                            :disabled=" detailInfo.SORT_ORD === '' || detailInfo.LVL === '1' "
                            class="pl-btn has-icon-only"
                            @click="srchDept(detailInfo.MNG_DEPT_NM)"
                            >
...
<!--                            />-->
<!--                            &lt;!&ndash; schRoleProp : 검색 조건 설정 CUSL : 상담사, MANAGER : 상담사를 제외한 LHCS직원 &ndash;&gt;-->
<!--                            &lt;!&ndash; PAGING_PROP : 페이징 사용여부 &ndash;&gt;-->
<!--                            &lt;!&ndash; chkYnProp : 체크박스 사용여부 / true : 리스트 , false : 오브젝트 리턴&ndash;&gt;-->
<!--                            &lt;!&ndash; searchInputProp : 입력 이름 &ndash;&gt;-->
<!--                          </template>-->
<!--                          <template slot="footer">-->
<!--                            <v-btn class="pl-btn is-trans" @click="[mixin_hideDialog('FindManager'), initFindMng()]">닫기</v-btn>-->
<!--                            &lt;!&ndash; <v-btn class="pl-btn" v-if="chkYnProp" @click="submitDialog">선택</v-btn> &ndash;&gt;-->
<!--                          </template>-->
<!--                        </compo-dialog>-->
<!--                      </v-dialog>-->
                      <v-dialog
                        ...
                        <compo-dialog header-title="조직 선택" @hide="mixin_hideDialog('OrgTree')">
                          <template slot="body">
                            <CompoTree ref="ognzDataCompo" :ExpandedProp=true @Close="dialogOrgTree = false" />
                          </template>
                          <template slot="footer">
...
export default {
 ...
 data() {
  ...
  async created() {
    ...
    //   this.setMngDept(managerInfo);
    // });
    this.$eventBus.$on("setOgnzTreeInfo", (ognz) => {
      this.setMngDept(ognz);
    });
  },
  ...
  methods: {
    ...
    async getTreeList() {
      ...
      let admYn = this.computedUserDetailType.includes('SYS') || this.computedUserDetailType.includes('ADMIN')
      let postParam = {
        ADM_YN : admYn ? "Y":"N"
      };
    ...
    srch_tree_act( node ){
      ...
      if( node != null && node.length > 0 ) {
        ...
        this.detailInfo.MNG_DEPT_ID = this.selectedNode.DEPT_ID;
        this.detailInfo.MNG_DEPT_NM = this.selectedNode.DEPT_NM;
    ...
    clearName(){
      ...
      this.detailInfo.MNG_DEPT_ID = "";
      this.detailInfo.MNG_DEPT_NM = "";
      this.detailInfo.MNG_CUSTCO_ID = "";
    },
    //사용자검색버튼 (개발용) 추후삭제
    srchDept(val){
      // this.mixin_showDialog('FindManager')
      this.mixin_showDialog('OrgTree')
    },
    setOgnzSubmitData() {
    ...
    // 관리자 정보  Set
    async setMngDept(data) {
      console.log('this.detailInfo.UP_KMS_CLSF_ID', this.detailInfo.UP_KMS_CLSF_ID)
      console.log('this.detailInfo.UP_KMS_CLSF_ID', this.detailInfo.UP_KMS_CLSF_ID === data.CUSTCO_ID)
    console.log(this.validateDept(data))
      if ( await this.validateDept(data)) {
        this.detailInfo.MNG_DEPT_ID = data.DEPT_ID;
        this.detailInfo.MNG_CUSTCO_ID = data.CUSTCO_ID;
        this.detailInfo.MNG_DEPT_NM = data.DEPT_NM;
      }else{
        this.clearName()
      }
    },
    async validateDept(deptInfo){
      let postParam = {
        CHK_CUSTCO_ID : deptInfo.CUSTCO_ID,
        CHK_DEPT_ID : deptInfo.DEPT_ID,
        UP_KMS_CLSF_ID : this.detailInfo.UP_KMS_CLSF_ID
      }
      let headParam = { head : { ns : 'lhcs.km.clsf.dao.kmClsfMapper'}}
      const response = await this.common_postCall('/api/biz/common/select/selectDeptOfClsf', postParam, headParam)
      if ( !response.HEADER.ERROR_FLAG ){
       if(response.DATA[0].CHK_DEPT_YN === 'Y'){
         return true;
       }else{
         this.showAlertCaution({msg : '상위 분류에 등록된 부서의 하위부서만 등록할 수 있습니다.'})
         return false;
        }
      }else{
        this.showAlertCaution({msg:'관리부서 유효성 체크 중 오류가 발생했습니다.'})
      }
    },
    clsfSaveValidate(){
    ...
    async clsfSave(){
      ...
      let postParam = {
        ...
        , MNG_DEPT_ID: this.detailInfo.MNG_DEPT_ID
        , MNG_CUSTCO_ID: this.detailInfo.MNG_CUSTCO_ID
