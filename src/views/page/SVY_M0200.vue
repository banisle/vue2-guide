...
export default {
  ...
  methods: {
    ...
    async checkTargetList(targetList) {
      ...
      //   const dupItem = targetList.slice(0, index+1).filter(filterItem => {
      //     return item.CUST_PHN_NO.replace(/-/gi, '') == filterItem.CUST_PHN_NO.replace(/-/gi, '')
      //   })
      //   if(dupItem.length != 1) {
      //     dupTargetList.push(item)
      //   }
      ...
      let alertMsg = '';
      // if(dupTargetList.length > 0) {
      //   alertMsg += '중복된 전화번호 참여자가 있습니다.<br/>';
      //   dupTargetList.map(item => {
      //     alertMsg += item.CUST_NM + ' / ' + item.CUST_PHN_NO + '<br/>'
      //   })
    ...
    //   //전체발송
    //   if(allFlag){
    //     cnt = this.gridDataText.length;
    //     tempGridData = _.cloneDeep(this.gridDataText);
    //   }else{  // 개별발송
    //     //선택항목이 없다면
    ...
    //       return;
    //     }
    //     cnt = this.gridSelectedData.length;
    //     tempGridData = _.cloneDeep(this.gridSelectedData);
    //   }

    //   this.showAlert(
    //     {
    //       alertDialogToggle: true,
    //       msg: `${cnt.toString()}건을 전송하시겠습니까?`,
    //       iconClass: 'is-info',
    //       type: 'confirm',
    //       callYes: async () => {
    //         const sUrl = "/api/mts/send/sendSMS";
    //         const headParam = {
    //           head: {
    //             'SERVICE' : 'mts.send.sendSMS',
    //             'METHOD' : 'sendSMS',
    //             'TYPE' : 'BIZ_SERVICE',
    //           }
    //         }
    ...
    //         // 수신자 목록
    //         const data = tempGridData;
    //         for(let i = 0; i < data.length; i++){
    //           let PHONE_NUMBER = [];
    //           PHONE_NUMBER.push(data[i].CUST_PHN_NO);

    //           const postParam = {
    //             CALLBACK_NUMBER : '01086263333',
    //             PHONE_NUMBER : PHONE_NUMBER,
    //             FILE_GROUP_KEY : '',
    //             SEND_DATE : this.$moment(new Date()).format("YYYYMMDDHHmmss"),
    //             MESSAGE : `설문조사 테스트 문자발송입니다. 접속링크 : https://palette.hkpalette.com/SVY?key=${data[i].URL_KEY}`
    //           }
    //           let resData = await this.common_postCall(sUrl, postParam, headParam );
    ...
    //           {
    //             //this.setErrMsg(resData);
    //             this.showAlert(`${this.MESSAGE.ERROR.ERROR} ${i.toString()}번째(${PHONE_NUMBER[0]})`);
    //             break;
    //           }
    //         }
