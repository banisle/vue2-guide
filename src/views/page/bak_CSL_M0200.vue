<template>
  <div
    ...
    <div class="pl-card-wrap" style="position: relative;">
      <div class="pl-card-top" style="border-bottom: 1px solid #dbdbdb !important">
      ...
      <div
        class="pl-card d-flex flex-grow-1 flex-column pt-0"
      ...
      <!-- slide modal 공통-->
      <v-slide-y-reverse-transition>
        <div class="pl-drop-layer is-mainchat-slide type02" v-if="slideChat === true" >
...
export default {
  ...
  async created() {
     ...
     * 채팅 메시지 인입
     */
    this.$eventBus.$on("busInoutCallbackD", (message) => {
  ...
  methods: {
    ...
    async chatHstSrch(chat){
      ...
      let postParam = {
        CHT_CUTT_ID: chat == 'BFR' ? (this.BFR_CHAT_LOG.length != 0 ? this.BFR_CHAT_LOG[0].BFR_CHT : this.CHAT_LOG[0].BFR_CHT) : (chat == 'AFTR' ? this.CHAT_LOG[this.CHAT_LOG.length-1].AFTR_CHT : this.chat_list_data.CLK_DATA.CHT_CUTT_ID),
        CUST_ID: this.chat_list_data.CLK_DATA.CUST_ID,
            ...
            if(phnNoConfirm){
              if(this.mixin_isEmpty(this.$store.getters['alertStore/GE_ALERT'].custPhnNo)){
                ...
                if(this.$store.getters['alertStore/GE_ALERT'].custPhnNo.replace(/[^0-9]/g, '').length < 11){
                  ...
                  this.iframeUrl = process.env.VUE_APP_VIDEO_URL + '/view/ssologin?loginId='+this.$store.getters['userStore/GE_USER_ROLE'].lgnId.replace('#', '%23')+'&tenantName=hkcloud&site='+this.$store.getters['userStore/GE_USER_ROLE'].company.SCHEMA_ID+'_'+this.$store.getters['userStore/GE_USER_ROLE'].company.CD+'&callid='+this.CHAT_CUTT_MODE+'_'+this.chat_list_data.CLK_DATA.CHT_CUTT_ID;
                  this.$store.commit("alertStore/hideAlert");
                }
              }
            }else{
