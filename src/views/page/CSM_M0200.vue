<template>
  ...
  <v-dialog
    ...
    >
      <template slot="body">
        <!-- component -->
        <CSM_M0200_DIALOG ref="CSM_M0200_DIALOG" :propsData="propsData" @refresh="selectDayTaskReportList" @setBtn="(data)=>{isFinal = data == 'Y'?true:false}"/>
      </template>
...
export default {
  ...
  methods: {
    ...
    clickReportBtn(item){
      ...
      this.mixin_showDialog('Report')
    },

    // 등록 버튼
    clickRegistBtn(){
      this.propsData.RPT_DAY = this.$moment().format("YYYY-MM-DD");
      this.propsData.SRCH_CUSTCO_ID = this.searchParams.SRCH_CUSTCO_ID;
      // this.dialogReport = true;
      this.mixin_showDialog('Report')
    },

    clickFinalBtn(){
      if(this.$refs.CSM_M0200_DIALOG) this.$refs.CSM_M0200_DIALOG.clickFinalBtn();
    },

    clickSaveBtn(){
      if(this.$refs.CSM_M0200_DIALOG) this.$refs.CSM_M0200_DIALOG.clickSaveBtn();
    }
  },
  mounted() {
    this.searchParams.SRCH_CUSTCO_ID = this.computedUserCenter;
    this.selectDayTaskReportList();
  },
  created() {
  },
