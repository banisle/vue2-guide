          ...
          <li>
            <div 
              ...
              <div class="pl-campaign-list-unit">
                ...
                <span :class="`ml-1 pl-round-chip is-sm ${ mixin_displayStatus(data.RSVT_RS_CD, `statList`) }`">{{ data.RSVT_RS_NM }}</span>
              </div>
              <div class="pl-campaign-list-unit">
            ...
            <div class="pl-campaign-list-btn" style="top: 18px">
              <v-icon
                :disabled="data.RSVT_RS_CD == 'CMPL' ? true : false"
  ...
  export default {
    ...
    data() {
      return{
        ...
        BOOKING_ID : "55185776-6B4F-43F1-8D9A-0BF9F8C7E27D",
        ANALYSIS_ID : "833489F5-311C-48A4-9DBA-93CD32C5CF19",
    ...
    methods: {
      ...
      async getRsvtList(){
        ...
        if(this.date){
          nowDate = this.date
          schDate = this.focus_cal ? this.focus_cal.replaceAll('-','') : nowDate.replaceAll('-','').replaceAll(':','').replaceAll(' ','').substr(0, 8)
        } else {
          nowDate = await this.mixin_getSvrDate("YYYYMMDDHH24MISS")
          schDate = this.focus_cal ? this.focus_cal.replaceAll('-','') : nowDate.substr(0, 8)
        }
        this.RESERVE_LIST = [];
        ...
        let postParam = {
          ...
          , SCH_ST_DT : schDate
          , SCH_END_DT : schDate
          , SCH_KEY : this.SCH_CUST ? 'CUST' : 'USER_ID'
          , SCH_KEYWORD : this.SCH_CUST ? this.SCH_CUST : this.$store.getters['userStore/GE_USER_ROLE'].userId
        }
