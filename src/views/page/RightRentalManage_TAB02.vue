  ...
  <div>
    ...
    <div class="pl-pager">
      ...
      <span class="pl-pager-period">
        보기 {{ mixin_getPagePeriod(items, page) }} / {{ totCnt }}
        <compo-tooltip-btn
          TitleProp="다음 검색"
          ClassProp="pl-tooltip-btn is-line"
          IconProp="pl-icon20 arrow-next-paging"
          TooltipPositionProp="bottom"
          @btnClick="srchMngofcPhnNo(false)"
        ></compo-tooltip-btn>
      </span>
    </div>
    ...
    <v-dialog
      ...
      <compo-dialog
        header-title="분양아파트 관리사무소 상세 정보"
        @hide="mixin_hideDialog('Edit')"
        @submit="submitDialog('Edit')"
      >
        <template slot="body">
          <RightRentalManage_TAB02_DIALOG
            ref="dialog"
            @srchMngofcPhnNo="srchMngofcPhnNo(true)"
          />
        </template>
...
<script>
import RightRentalManage_TAB02_DIALOG from '@/views/page/RightRentalManage_TAB02_DIALOG'

export default {
  name: 'RightRentalManage_TAB02', //분양아파트 관리사무소 전화번호
  components: {
    RightRentalManage_TAB02_DIALOG
  },
  data() {
    return {
      ...
      headers: [
        ...
        { text: '본부', value: 'HDQT_NM', align: 'left', width: '5%', sortable: false },
        { text: '지구명', value: 'DSTRCT_NM', align: 'left', width: '12%', sortable: false },
        { text: '단지명', value: 'DANJI_NM', align: 'left', width: '', sortable: false },
        { text: '입주일', value: 'HUSIN_DAY', align: 'center', width: '12%', sortable: false },
        { text: '관리소 전화번호', value: 'MNGOFC_PHN_NO', align: 'center', width: '12%', sortable: false },
        { text: '세대수', value: 'HH_CNT', align: 'center', width: '7%', sortable: false },
  ...
  methods: {
    ...
    async openDel(item) {
      ...
      let postParam = {
        MNGOFC_PHN_NO_ID: item.MNGOFC_PHN_NO_ID,       //본부
      }
      let headParam = {
        head: {
          ns : 'lhcs.rentalHusMng.MngofcPhnNo.dao.MngofcPhnNoMapper'
          , sn : 'deleteMngofcPhnNo'
        }
      }
      ...
      if(!resData.HEADER.ERROR_FLAG){
        ...
        this.srchMngofcPhnNo(false);
      }
    },

    //분양아파트 관리사무소 전화번호 조회
    async srchMngofcPhnNo(next){
      if( !next ){
        this.items = [];
      ...
      if(!resData.HEADER.ERROR_FLAG){
        ...
        this.items = [...this.items, ...resData.DATA];
    ...
    //저장버튼 클릭
    strg(){
      this.$refs.dialog.stngMngofcPhnNo();
    ...
    initDialog(){
      if(this.$refs.dialog){
        this.$refs.dialog.initMngofcPhnNo();
      } else {
        setTimeout(() => {
          this.$refs.dialog.initMngofcPhnNo();
        }, 100);
    ...
    openDialog(item){
      if(this.$refs.dialog){
        this.$refs.dialog.setMngofcPhnNo(item);
      } else {
        setTimeout(() => {
          this.$refs.dialog.setMngofcPhnNo(item);
        }, 100);
