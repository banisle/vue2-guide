...
export default {
  ...
  methods: {
    ...
    async getExpsnAttr(){
      ...
      let sUrl = '/api/setting/customer/info-expsn/expsninfo-inqire/inqire';
      let postParam = {
        SE: 'CUSTOM',
      };
      let headParam = {
      ...
      // console.log(response.EXPSN_ATTR);

      if(!response.HEADER.ERROR_FLAG){

        // this.GRID_EXPSN_ATTR = _.cloneDeep(response.EXPSN_ATTR);
        this.GRID_EXPSN_ATTR = Object.assign(response.EXPSN_ATTR);

        for (let k in response.EXPSN_ATTR) {
          const maxBytes = new TextEncoder().encode(response.EXPSN_ATTR[k].EXPSN_ATTR_NM);
          this.CUST_GRID_HEADERS.push({
            text: response.EXPSN_ATTR[k]["EXPSN_ATTR_NM"],
            value: response.EXPSN_ATTR[k]["EXPSN_ATTR_COL_ID"],
            // width: response.EXPSN_ATTR[k].EXPSN_ATTR_NM.length > 6 ? '120px' : '80px',
    ...
    */
    ...
    async getCustInfoList(next) {
      ...
      let sUrl = '/api/biz/common/select/custList';
      ...
      if (!response.HEADER.ERROR_FLAG) {
        this.CUST_GRID_HEADERS = [
          ...
          {text: '고객명', value: 'CUST_NM', width: '100px'},
          {text: '고객 ID', value: 'CUST_ID', width: '80px'},
          {text: '고객 전화번호', value: 'CUST_PHN_NO', width: '130px'},
        ];
        ...
        this.CUST_GRID_HEADERS.push({text: '최초 등록일시', value: 'REG_DT', align: 'center', width: '150px', sortable : false});
        this.CUST_GRID_HEADERS.push({text: '마지막 수정일시', value: 'MDFCN_DT', align: 'center', width: '150px', sortable : false});
        ...
        for (let i in this.EXPSN_ATTR) {
          ...
          if (this.EXPSN_ATTR[i].DATA_TYPE_CD === 'DAT') {//날짜형인 경우
            ...
            if (this.EXPSN_ATTR[i].ATTR_VL) {
              ...
              // console.log(this.EXPSN_ATTR[i].DATA_LEN)
            } else {
              this.EXPSN_ATTR[i].V_MODEL_DTL = [];
    ...
    async downloadExcel(){
      ...
      this.$cookies.set("excelDnwldCustList", excelDnwldCustList, "30d", "/", "", true, "Lax");
