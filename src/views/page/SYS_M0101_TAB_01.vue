    ...
    <div class="is-col-fix is-vrt" style="width: 50%">
            ...
            <div class="pl-form-inline">
              ...
              <div class="pl-desc">
                <v-select
                  ...
                  @change="srch_commn_cd()"
                ></v-select>
                <v-text-field
                  class="pl-form type-middle is-search is-lg"
      ...
      <div class="pl-card-body is-border-top">
        ...
        <template v-slot:item.SYS_CD_YN="{ item }">
          <span
            :class="`pl-badge is-${mixin_displayStatus(item.SYS_CD_YN, 'STAT_LIST2')}`"
            style="width: 60px;"
            >
            {{ item.SYS_CD_YN === 'Y' ? '시스템용' : '센터용 '}}
          </span>
        </template>
        ...
        <div class="pl-grid-top pt-2">
          <div class="pl-grid-top-left">
            ...
            <v-btn class="pl-btn is-icon is-trans"
              ...
              :disabled="$store.getters['userStore/GE_USER_ROLE'].userId == '4' ? false : (this.SEL_COMM_CD.SYS_CD_YN === 'Y' ? true : JSON.stringify(SEL_COMM_CD) != '{}' ? false : true)"
            >
            <!-- :disabled="$store.getters['userStore/GE_USER_ROLE'].userId == '4' ? false : (this.SEL_COMM_CD.SYS_CD_YN === 'Y' ? true : JSON.stringify(SEL_COMM_CD) != '{}' ? false : true)" -->
              <span class="pl-icon20 circle-plus"></span>
              등록
            ...
            <v-btn class="pl-btn is-icon is-trans"
              @click="clickDel('DTL')"
              :disabled="CHC_GROUP_CD_DTL_LIST.length != 0 ? ($store.getters['userStore/GE_USER_ROLE'].userId == '4' ? false : (this.SEL_COMM_CD.SYS_CD_YN === 'Y' ? true : false)) : true"
            >
              <span class="pl-icon20 trash"></span>
              삭제
            </v-btn>
    ...
    <v-dialog
      ...
      >
            ...
            <div class="pl-form-inline">
              ...
              <div class="pl-desc">
                <v-select
                  ...
                  :readonly="STRG_DTL == 'DTL' ? true : false"
                ></v-select>
                <!-- :readonly="$store.getters['userStore/GE_USER_ROLE'].userId == '4'  ? false : (STRG_DTL == 'DTL' ? true : false)" -->
              </div>
            </div>
            <div class="pl-form-inline">
              <span class="pl-label">
        ...
        <template slot="footer">
          <v-btn class="pl-btn is-trans" @click="[mixin_hideDialog('RegCommonCode'), init()]">닫기</v-btn>
          <v-btn class="pl-btn" @click="strg_commn_cd(STRG_DTL,REG_MDFCN)" :disabled="REG_MDFCN == 'MDFCN' ? ($store.getters['userStore/GE_USER_ROLE'].userId == '4'  ? false : (CD_TY=='SYS' ? true : false)) : DPCN_CHK">저장</v-btn>
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      CUSTCO_LIST:[],                     //업무구분(센터) 리스트
      SCH_CUSTCO_ID:'-1',                  //검색업무구분
      CUSTCO_ID: '-1',                      //업무구분(센터)
      ...
      ROW_PER_PAGE: -1,
      headers: [
        { text: '번호', value: 'SORT_ORD', align: 'center', width: '50px' },
        { text: '분류코드', value: 'CD_ID'},
        { text: '분류코드명', value: 'CD_NM', width: '250px'},
        { text: '업무그룹', value: 'CO_NM', width: '120px' },
        { text: '코드유형', value: 'SYS_CD_YN', align: 'center', width: '100px'},
        { text: '사용여부', value: 'USE_YN', align: 'center', width: '100px'},
      ],
      headers_dtl: [
        { text: '번호', value: 'SORT_ORD', align: 'center', width: '70px' },
        { text: '공통코드', value: 'CD_ID', },
        { text: '공통코드명', value: 'CD_NM', width: '250px'},
        { text: '사용여부', value: 'USE_YN', align: 'center', width: '100px'},
      ],
      // 상태 컬러
      STAT_LIST: [
      ...
      CD_TY:'CENTER',           //시스템(SYS) : 회사구분 없음 , 고객용(CENTER) : 회사구분 있음
      COMM_CD:'',               //분류코드
      CD_NM:'',                 //코드명
      CD_VL1:'',                 //코드값1
      CD_VL2:'',                 //코드값2
      ...
      MESSAGE : {
        ...
        ALERT : {
          REG_SUCCESS : { alertDialogToggle: true, msg: '공통코드가 저장되었습니다', iconClass: 'is-done', type: 'default' }
        ...
        TOAST : {
          REG_SUCCESS: {  msg: '공통코드가 저장되었습니다.', class: 'success', hasToastIcon: true, icon: 'mdi-checkbox-marked-circle' , time: 2000 }
  ...
  methods: {
    ...
    async strg_commn_cd(type,reg){
      ...
      let postParam = {
        ...
        CO_ID: this.CUSTCO_ID,
        ORG_CO_ID: reg == 'MDFCN' ? (type == 'DTL' ? this.SEL_COMM_DTL_CD.CUSTCO_ID : this.SEL_COMM_CD.CUSTCO_ID) : '',
        GROUP_CD_ID : type == 'DTL' ? this.SEL_COMM_CD.CD_ID : '*',
        CD_ID: this.COMM_CD,
    ...
    click_dtl_strg(){
      ...
      // this.CD_TY='CENTER';
      this.CD_TY=this.SEL_COMM_CD.SYS_CD_YN == 'Y' ? 'SYS' : 'CENTER'
      this.CUSTCO_ID=this.SEL_COMM_CD.CUSTCO_ID
      this.COMM_CD='';
      this.CD_NM='';
      this.CD_VL1='';
      this.CD_VL2='';
    ...
    click_mdfcn(){
      ...
      this.CD_VL1=this.SEL_COMM_CD.CD_VL1;
      this.CD_VL2=this.SEL_COMM_CD.CD_VL2;
      this.CD_VL3=this.SEL_COMM_CD.CD_VL3;
      this.CD_VL4=this.SEL_COMM_CD.CD_VL4;
    ...
    click_dtl_mdfcn(){
      ...
      this.CD_TY=this.SEL_COMM_CD.SYS_CD_YN == 'Y' ? 'SYS' : 'CENTER';
      this.CUSTCO_ID=this.SEL_COMM_DTL_CD.CUSTCO_ID
      this.COMM_CD=this.SEL_COMM_DTL_CD.CD_ID;
      this.CD_NM=this.SEL_COMM_DTL_CD.CD_NM;
      this.CD_VL1=this.SEL_COMM_DTL_CD.CD_VL1;
      this.CD_VL2=this.SEL_COMM_DTL_CD.CD_VL2;
      this.CD_VL3=this.SEL_COMM_DTL_CD.CD_VL3;
      this.CD_VL4=this.SEL_COMM_DTL_CD.CD_VL4;
    ...
    clickDel(type){
      if(type == 'DTL'){
        if(this.CHC_GROUP_CD_DTL_LIST.length > 1){
          this.showAlert(this.MESSAGE.ERROR.DEL_ONLY_ONE)
        } else if(this.CHC_GROUP_CD_DTL_LIST.length == 1) {
          this.showAlert(this.MESSAGE.CONFIRM.DEL_DTL)
        }
      } else {
        if(this.CHC_GROUP_CD_LIST.length > 1){
          this.showAlert(this.MESSAGE.ERROR.DEL_ONLY_ONE)
        } else if(this.CHC_GROUP_CD_LIST.length == 1) {
  ...
  async created() {
    ...
    // console.log("CD_TY_DROP", this.CD_TY_DROP);
    this.CD_TY_DROP = [
      {
        text: '시스템용',
        value: 'SYS'
    ...
    //업무구분(센터) 리스트
    // this.CUSTCO_LIST = []
    // for(let i=0;i<this.$store.getters["userStore/GE_USER_ROLE"].company_list.length;i++){
    //   this.CUSTCO_LIST.push({
    //     text: this.$store.getters["userStore/GE_USER_ROLE"].company_list[i].CD_NM,
    //     value: this.$store.getters["userStore/GE_USER_ROLE"].company_list[i].CD
    //   })
    // }
    this.CUSTCO_LIST = await this.mixin_custco_list_combo();
  },
  computed: {
  },
  watch: {
