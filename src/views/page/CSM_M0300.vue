<template>
          ...
          <div class="pl-form-inline" v-if="computedUserType === 'ETC_ADMIN' || computedUserType === 'LEADER'">
            ...
            <div class="pl-desc">
              <v-select
                class="pl-form type-middle is-md"
                v-model="SCH_DEPT_ID"
                :items="mixin_ognz_list_detail(common_ognz_list, SCH_CUSTCO_ID.DEPT_ID, '전체', 'Y'/*콜센터여부,없으면전체*/ )"
                no-data-text="결과 없음"
      ...
      <div class="pl-grid-top pt-2">
        ...
        <div class="pl-grid-top-utils">
          ...
          <compo-excel
            ...
            :DataBodyProp="items"
            FileNameProp="대내외 민원접수 현황"
            SheetNameProp="대내외 민원접수 현황"
          />
        </div>
      </div>
      <!-- grid -->
  ...
  <v-slide-x-reverse-transition>
    <div
      ...
      <!-- component -->
      <CTM_M0300_TAB_02_DETAIL refs="CTM_M0300_TAB_02_DETAIL" @Close="dialogDetail = false" :popData="popData" @refresh="clickSearchBtn"/>
    </div>
   </v-slide-x-reverse-transition>
 </div>
</template>
...
export default {
  name: "MENU_CSM_M0300", //대 내/외 민원 접수 현황
  components: {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: '민원구분', value: 'CVLCPT_SE_NM', align: 'center', width: '100px', sortable: true },
        { text: '접수일', value: 'REG_DT', align: 'center', width: '100px', sortable: true },
        { text: '상담일', value: 'CUTT_DT', align: 'center', width: '100px', sortable: true },
        { text: '센터', value: 'RCPT_CUSTCO_NM', align: '', width: '110px', sortable: true },
        { text: '소속', value: 'RCPT_DEPT_NM', align: '', width: '110px', sortable: true },
        { text: '상담사', value: 'CUSL_NM', align: '', width: '110px', sortable: true },
        { text: '접수경로', value: 'CVLCPT_RCPT_PATH_NM', align: '', width: '110px', sortable: true },
        { text: '고객명', value: 'CUST_NM', align: '', width: '90px', sortable: true },
        { text: '고객 연락처', value: 'CUST_PHN_NO', align: 'center', width: '110px', sortable: true, telYn : true },
        { text: '민원분류1', value: 'CVLCPT_CLSF_NM0', align: 'center', width: '100px', sortable: true },
        { text: '보상여부', value: 'CPST_YN_NM', align: 'center', width: '100px', sortable: true },
        { text: '진행상태', value: 'PRGRS_STTS_CD_NM', align: 'center', width: '100px', sortable: true },
        { text: '최종 처리일', value: 'LAST_PRCS_DT', align: 'center', width: '100px', sortable: true },
        { text: '최종 처리자', value: 'LAST_PRCR_NM', align: '', width: '100px', sortable: true },
      ],
      items: [],
      checkedItem:[],
      gridTotalCnt : 0,
  ...
  async mounted() {
    ...
    this.list_common_code = await this.mixin_common_code_get_all(codeName, 'Y');
    ...
    if(this.computedUserType === 'ETC_ADMIN') {
      this.SCH_CUSTCO_ID = centerObj.length > 0 ? centerObj[0]: "";
      this.SCH_DEPT_ID = "";
    } else if( this.computedUserType === 'LEADER' ) {
      this.SCH_CUSTCO_ID.CUSTCO_ID = this.computedUserCenter;
      this.SCH_CUSTCO_ID.DEPT_ID = this.computedUserCenterDeptId;
      if( this.computedUserDetailType.indexOf("CENTER_LEADER") < 0) {  //센터장이 아닌경우 자기자신부서만.
    ...
    this.clickSearchBtn(); //민원 접수 목록
  },
  beforeDestroy(){
    this.$eventBus.$off("schDtCvlcptList"); //eventBus 중복방지를 위해 off
  },
  async created() {
    this.$eventBus.$on("schDtCvlcptList", (item) => {
      console.log( item );
      this.SCH_ST_DTS = this.mixin_convertDate(item.SCH_ST_DT, 'yyyy-MM-dd');
      this.SCH_END_DTS = this.mixin_convertDate(item.SCH_END_DT, 'yyyy-MM-dd');
      this.SCH_PRGRS_STTS_CD = 'P'; //진행중
  ...
  methods: {
    ...
    clickSearchBtn(){
      if(this.mixin_isEmpty(this.SCH_ST_DTS) || this.mixin_isEmpty(this.SCH_END_DTS)){
    ...
    async selectCvlcptList(next) {
      ...
      let postParam = {
        ...
        , SCH_CUSTCO_ID: this.SCH_CUSTCO_ID.CUSTCO_ID
