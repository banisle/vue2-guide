...
export default {
  ...
  methods: {
    ...
    loadSrvyItem() {
      if(this.mixin_isEmpty(this.selectedRow.SRVY_QITEM_ID)) {
    ...
    async setQuSaveYes(){
      ...
      let arrItemList = [];
      this.qitem.CHC_LIST.forEach((e, i) => {
        arrItemList.push({
          QITEM_CHC_ID : '',
          SRVY_QITEM_ID : '',
          QITEM_CHC_CN : e.QITEM_CHC_CN,
          CSORT_ORD : (i+1).toString(),
          MVMN_SRVY_QITEM_GROUP_ID: this.gQuData.GROUP_MVMN_USE_YN && e.CHC_MVMN_SRVY_QITEM_GROUP_ID ? e.CHC_MVMN_SRVY_QITEM_GROUP_ID : '',
