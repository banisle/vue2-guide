      ...
      <div class="pl-grid-top pt-2">
        <div class="pl-grid-top-left spacing-wrap sp-16 ">
          <div class="spacing-wrap sp-8">
            ...
            <div>
              <span class="pl-bullet-txt is-red">철회</span>
              <strong >({{ WITHDRAWAL_CNT }})</strong>
            </div>
            <div>
              <span class="pl-bullet-txt is-org">반려</span>
              <strong >({{ RETURN_CNT }})</strong>
            </div>
          </div>
        </div>
...
export default {
  name: 'CTM_M0200_TAB_02', //배려고객 관리 대장
  components: {
    StatusBoardSlideDetail
  },
  data() {
    return {
      ...
      headers: [
        ...
        { text: '센터', value: 'CUSTCO_NM', align: 'left', width: '8%', sortable: false },
        { text: '상담사', value: 'CUSL_NM', align: 'left', width: '5%', sortable: false },
        { text: '기안자', value: 'GIVER_NM', align: 'left', width: '5%', sortable: false },
        { text: '고객 전화번호', value: 'CUST_PHN_NO', align: 'left', width: '7%', sortable: false, telYn: true },
        ...
        { text: '처리결과', value: 'CAUTION_CUST_STTS_NM', align: 'center', width: '7%', sortable: false },
        { text: '철회사유', value: 'WDRW_RSN_NM', align: 'left', width: '', sortable: false },
        { text: '차단 시작일', value: 'CUTOFF_BGNG_DT', align: 'center', width: '6%', sortable: false },
        { text: '차단 종료일', value: 'CUTOFF_END_DT', align: 'center', width: '6%', sortable: false },
        { text: '차단기간', value: 'IVR_CUTOFF_PERIOD_CD', align: 'center', width: '5%', sortable: false },
        { text: '휴게부여', value: 'CUSL_RST_YN', align: 'center', width: '5%', sortable: false },
        { text: '휴게시간', value: 'CUSL_RST_HR', align: 'center', width: '5%', sortable: false },
        { text: '심리상담', value: 'CUSL_PCLG_CUTT_YN', align: 'center', width: '5%', sortable: false },
      ],
      items: [],
  ...
  beforeDestroy(){
    this.$eventBus.$off("getCustCautionList2"); //eventBus 중복방지를 위해 off
  },
  async created() {
    //공통코드설정
    ...
    this.$eventBus.$on("getCustCautionList2", () => {
  ...
  methods: {
    ...
    async getCustCautionList(next) {
      ...
      if (!response.HEADER.ERROR_FLAG){
        ...
        // this.WITHDRAWAL_CNT = 0; //철회
        // this.RETURN_CNT = 0; //반려
        ...
        //   else if(data.CAUTION_CUST_STTS_CD === 'REQ05') this.WITHDRAWAL_CNT++;
        //   else if(data.CAUTION_CUST_STTS_CD === 'REQ06') this.RETURN_CNT++;
        // });

        this.TEMP_CUTOFF_CNT = response.LABEL_INFO[0].CNT; //임시차단
        this.CUTOFF_CNT = response.LABEL_INFO[3].CNT; //정식차단
        this.WITHDRAWAL_CNT = response.LABEL_INFO[4].CNT; //철회
        this.RETURN_CNT = response.LABEL_INFO[5].CNT; //반려
      }
