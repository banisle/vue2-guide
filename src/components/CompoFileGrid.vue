<template>
  <div>
    <div :class="`pl-grid-top pl-0 pr-0 ${NoMarginProp ? '' : 'is-mt-m'}`">
      <div class="pl-grid-top-left">
        <strong class="pl-filegrid-title" :class="SmallTitleProp ? 'is-small' : ''">{{ TitleProp }}({{FilesProp.length}})</strong>
        <v-icon
          v-show="RequiredProp"
      ...
      <div class="pl-grid-top-utils" v-if="HeaderButtonProp">
        <v-btn class="pl-btn is-icon is-trans" @click="addFile" :disabled="DisabledProp">
          <span class="pl-icon20 circle-plus"></span>
          추가
        </v-btn>
        <v-btn class="pl-btn is-icon is-trans" @click="removeFile" :disabled="DisabledProp">
    ...
    <v-data-table
      ...
      :class="`pl-grid mt-1 ${HeaderButtonProp ? 'has-control' : null}`"
      :headers="HeadersProp"
      :items="FilesProp"
      fixed-header
      item-key="index"
      ...
      <template v-slot:item.download="{ item }">
        <!-- <v-btn
          ...
          <v-icon
            ...
            class="pl-icon20 file-down-arrow"></v-icon>
      </template>
    </v-data-table>
