  ...
  <div>
    <div class="pl-header">
      ...
      <div class="spacing-wrap sp-4 ml-auto">
        <v-btn class="pl-btn is-sub is-sm" @click="clickInitBtn">전체 초기화 </v-btn>
        <v-btn
            ...
            <tr>
              ...
              <td>
                <v-text-field
                  ...
                  placeholder="검색어 입력"
                  @keydown.enter="searchSchedulerListPaging()"
                >
              ...
              <td colspan="3">
                <div class="spacing-wrap sp-4">
                  ...
                  <v-text-field
                    v-model="SRCH_TEXT"
                    class="pl-form is-auto"
                    @keyup="setPatternText(SRCH_TEXT)"
                  ></v-text-field>
...
export default {
  name: "CompoAsSchedulerMobile_INFO", //스케쥴러 정보
  props: {
    IsExpandSchListProp: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    async searchSchedulerListPaging() {
      ...
      const searchCondition = {
        ...
        SRCH_TEXT: this.SRCH_TYPE === "MBL_NO" ? this.SRCH_TEXT.replace(/[^0-9]/g, "") : this.SRCH_TEXT,  //검색어
        ...
        SRCH_STATUS: this.SRCH_STATUS, //방문예정일 등록 리스트
    ...
    setPatternText(val) {
      console.log("setPhoneNo", val, this.SRCH_TYPE)
      if(this.SRCH_TYPE === "MBL_NO") {
        this.SRCH_TEXT = this.mixin_setPhoneNo(val);
      } else if (this.SRCH_TYPE === "V_FLW_ACP_SN") {
        this.SRCH_TEXT = this.mixinAsSetAsVFlwAcpSn(val);
      } 
    },
    ...
    initInfoData(){
      ...
      //시작일
      this.START_DT = (new Date(Date.now() - (1 * 24 * 60 * 60 * 1000) - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10);
      //종료일
      this.END_DT = (new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10);
    ...
    clickInitBtn(){
      this.initInfoData();
      this.$eventBus.$emit('initAsSchedulerMobile','');
    },
