    ...
    <div
      ...
      <div class="pl-pager-row">
        ...
        <v-select
          ...
          @change="ROW_PER_PAGE = parseInt($event, 10); searchSchedulerListPaging()"
        ></v-select>
      </div>
      <v-pagination
        v-model="page"
...
export default {
  name: "CompoAsSchedulerMobile_GRID", //스케쥴러 정보
  components: {
    CompoAsSchedulerAllApply
  },
  props: {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'AS접수번호', value: 'V_FLW_ACP_SN', align: 'left', width: '115px', sortable: true },
        
        { text: '진행현황', value: 'MSTATUS', align: 'center', width: '65px', sortable: true },
        { text: '제외사유', value: 'EXCATE', align: 'left', width: '65px', sortable: true },
        { text: '보수지시일', value: 'RPR_NSTC_DT', align: 'center', width: '6%', sortable: true },
        ...
        { text: '관리소', value: 'MGOF_CNT', align: 'center', width: '4.5%', sortable: true },
      ...
      table_head: [
        ...
        { text: '진행현황', value: 'MSTATUS', align: 'center', width: '90', sortable: true },
        { text: '제외사유', value: 'EXCATE', align: 'left', width: '90', sortable: true },
        { text: '보수지시일', value: 'RPR_NSTC_DT', align: 'center', width: '120', sortable: true },
  ...
  created() {
    ...
    // 전체초기화
    this.$eventBus.$on('initAsSchedulerMobile',(cuslInfo)=> {
      this.initGridData();
    });
  },
  beforeDestroy() {
    ...
    this.$eventBus.$off("initAsSchedulerMobile");
  },
  mounted() {
    if(this.IsPopupProp) {
      if(window.opener) {
  ...
  methods: {
    ...
    async searchSchedulerListPaging(next = '') {
      ...
      //모바일 스케줄러에서 호출 시 쿼리 변경 있음.
      postParam.MOBILE_YN = this.MOBILE_YN;
      ...
      if (!response.HEADER.ERROR_FLAG) {
        ...
        if(this.searchCondition.SELECT_ROW_DBL_YN && this.searchCondition.SELECT_ROW_DBL_YN == "Y" && this.SCHEDULER_LIST.length > 0) {
          this.searchCondition.SELECT_ROW_DBL_YN = 'N';
          this.SEL_ROW = this.SCHEDULER_LIST[0];
          if(!this.mixin_isEmpty(this.SEL_ROW.ARA_HDQ_CD) && !this.mixin_isEmpty(this.SEL_ROW.FLW_RGS_DT) && !this.mixin_isEmpty(this.SEL_ROW.FLW_ACP_SN) && !this.mixin_isEmpty(this.SEL_ROW.FLW_DTL_SN)) {
    ...
    initGridData(){
      ...
      // 태그지정
      this.DropTags = false;
      this.addTagWrap = false;
      this.newTagTxt = '';
      this.editingIndex = null;
      this.tagPicker = null;
      this.dropChgClr = false;
      this.newTagClr = '#333';
