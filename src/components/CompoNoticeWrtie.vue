        ...
        <div class="pl-form-inline" v-show="FORM_DATA.BBS_TYPE_CD != 'GNRL_BBS'/*일반*/">
          ...
          <div class="pl-desc">
            <compo-date-range-picker
              ...
              :StartDayProp.sync="PSTG_BGNG_DT_DD"
              :EndDayProp.sync="PSTG_END_DT_DD"
              @startDayChange="dateChange($event, 'PSTG_BGNG_DT_DD')"
              @endDayChange="dateChange($event, 'PSTG_END_DT_DD')"
...
import Vue from "vue";
export default {
  name: "CompoNoticeWrtie", //name은 'MENU_' + 파일명 조합
  props: {
    TypeProp: {
      type: String,
      default: 'notice'
  ...
  methods: {
    ...
    initFormData() {
      this.FORM_DATA = {};
      this.CUSTCO_ID = this.computedUserCenter;
      this.PSTG_BGNG_DT_DD = this.$moment(new Date()).format('YYYY-MM-DD');
      this.PSTG_END_DT_DD = '2099-12-31';
    ...
    async saveProcess() {
      ...
      let postParam = JSON.parse(JSON.stringify(this.FORM_DATA))/*DeepCopy*/;
      ...
      console.log( this.PSTG_BGNG_DT_DD, this.PSTG_END_DT_DD );
      postParam.PSTG_BGNG_DT = this.PSTG_BGNG_DT_DD.replaceAll("-","");
      postParam.PSTG_END_DT = this.PSTG_END_DT_DD.replaceAll("-","");
      ...
      if (!response.HEADER.ERROR_FLAG) {
        this.showToastInfo({msg : '정상 처리되었습니다.', callAfter : ()=>{
            ...
            if( !this.mixin_isEmpty(postParam.PST_ID ) ) {
              this.$eventBus.$emit('reloadBoardDetail', postParam);
            }
        }});
      }else {
    ...
    dateChange(event, itemName) {
      if(itemName == "PSTG_BGNG_DT_DD") {
        this.PSTG_BGNG_DT_DD =  event;
      } else {
        this.PSTG_END_DT_DD = event;
      }
    },
  },
  mounted() {
  ...
  async created() {
    ...
    if( this.FORM_DATA.PSTG_BGNG_DT_DD) this.PSTG_BGNG_DT_DD = this.FORM_DATA.PSTG_BGNG_DT_DD;
    if( this.FORM_DATA.PSTG_END_DT_DD) this.PSTG_END_DT_DD = this.FORM_DATA.PSTG_END_DT_DD;
    ...
    if( this.FORM_DATA.BBS_TYPE_CD === 'EMRG_NTC'/*긴급공지인경우*/ ) {
      ...
      this.SRCH_PARAMS_TOP.SRCH_DEPT.value = this.computedUserCenterDeptId;
      this.SRCH_PARAMS_TOP.SRCH_DEPT.DEPT_ID = this.computedUserCenterDeptId;
