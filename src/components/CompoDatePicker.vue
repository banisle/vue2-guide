    ...
    <v-text-field
      ...
      <template v-slot:append>
        <v-menu
          ...
          nudge-left="270"
          min-width="340"
          :content-class="`pl-calendar firstday${FirstDay} calendar-type-${PickerType}`"
          :disabled="DisabledProp"
        >
          <template v-slot:activator="{ on, attrs }">
            <v-icon
...
export default {
  ...
  methods: {
    ...
    clearDateTimes(){
      this.$emit('update:DateProp', "");
      this.setTimeHours(0);
      this.setTimeMinutes(0);
      if(!this.mixin_isEmpty(this.selectedTimeMinutes)) this.setTimeSeconds(0);
    },
  },
  computed: {
