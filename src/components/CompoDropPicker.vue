<template>
 <div class="pl-dateDropWrap">
    ...
    <div
      ...
      :style="{ minWidth: '120px', borderRadius: '12px', position: 'fixed', top: `${posY}px`, left: `${posX}px` }"
      >
      <div class="pl-drop-layer-body">
        <ul class="pl-drop-layer-list">
          <li
            v-for="(time, index) in RangeDropPicker" :key="index"
            class="d-flex"
...
export default {
  ...
  computed: {
    ...
    computedEndDate: {
      ...
      set(newDate){
        if(this.BbsDateProp){
          this.$emit("update:StartDateProp", this.currentDate);
          this.$emit("update:EndDateProp", newDate);
        }else{
          this.$emit("update:StartDateProp", newDate);
          this.$emit("update:EndDateProp", this.currentDate);
        }
