<template>
  <div class="guide-content">
    ...
    <div class="guide-card">
      ...
      <v-data-table
        :class="`pl-grid is-mt-s ${ toggleGridClass ? 'is-hover' : '' }`"
        :headers="headers"
        :items="items"
        fixed-header
        item-key="index"
    ...
    <h2>{{ toplink[1] }}</h2>
    <div class="guide-card">
      <strong class="is-txt-red">그리드 show-select 속성 있을때, pl-grid 클래스에 has-control 클래스 추가</strong>
    ...
    <div class="guide-card">
      <v-data-table
        ...
        <template v-slot:item.type="{ item }">
          <span
            :class="`pl-badge is-${mixin_displayStatus(item.type, 'STAT_LIST')}`"
            >
          {{ item.type }}</span>
        </template>
    ...
    <h2>{{ toplink[11] }}</h2>
    <div class="guide-card">
      <p>colResizing클래스 추가시 resizing기능 추가, mounted에서 호출 </p>
      <v-data-table
        class="pl-grid colResizing"
        :headers="headers7"
        :items="items"
        :items-per-page="30"
        item-key="index"
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: '헤더 정렬 변경', value: 'subject', align: 'center text-left-important', sortable: false},
        { text: '헤더 정렬 변경', value: 'subject', align: 'center text-center-important', sortable: false},
        { text: '헤더 정렬 변경', value: 'subject', align: 'center text-right-important', sortable: false},
  ...
  data () {
    return {
      ...
      headers3: [
        ...
        { text: '수정', value: 'edit', align: 'center', width: '80px', sortable: false },
        { text: '삭제', value: 'delete', align: 'center', width: '80px', sortable: false },
      ],
      items: [
        {
          index: 1,
