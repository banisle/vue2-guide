...
$.extend($.jgrid,{
	...
	getCellIndex : function (cell) {
		...
		if ($.newBrowser.ie) { return $.inArray(c, c.parentNode.cells); }
		return c.cellIndex;
	},
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var isMSIE;
		if($.newBrowser) {
			isMSIE = $.newBrowser.ie ? true:false;
		} else{isMSIE = false; }
		...
		constructTr = function(id, hide, altClass, rd, cur, selected) {
			...
			if(!$.isEmptyObject( rowAttrObj )) {
				...
				try { delete rowAttrObj.role; } catch(ra){console.log(ra)}
		...
		$(grid.bDiv)
			.append($('<div style="position:relative;'+(false ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
...
open:function(s,t){var h=H[s],c=h.c,cc='.'+c.closeClass,z=(parseInt(h.w.css('z-index')));z=(z>0)?z:3000;var o=$('<div></div>').css({height:'100%',width:'100%',position:'fixed',left:0,top:0,'z-index':z-1,opacity:c.overlay/100});if(h.a)return F;h.t=t;h.a=true;h.w.css('z-index',z);
 ...
 if(ie6){try{$('html,body').css({height:'100%',width:'100%'});if(o){o=o.css({position:'absolute'})[0];for(var y in {Top:1,Left:1})o.style.setExpression(y.toLowerCase(),"(_=(document.documentElement.scroll"+y+" || document.body.scroll"+y+"))+'px'");}}catch(__){console.log("exception")}}
...
f=function(h){try{$(':input:visible',h.w)[0].focus();}catch(_){console.log("excepiton")}},
...
i=function(e,h,k,aR){
	return e.each(function(){
		...
		h.bind('mousedown',{e:e,k:k},function(v){
			...
			if(E.css('position') != 'relative'){try{E.position(p);}catch(e){console.log(e)}}
			M={
				X:p.left||f('left')||0,
				Y:p.top||f('top')||0,
...
//Helper functions
	...
	viewModal : function (selector,o){
		...
		if ($.fn.jqm && o.jqm === true) {
			...
			try{$(':input:visible',selector)[0].focus();}catch(_){console.log("exception")}
		}
	},
							...
							complete:function(data,status){
								...
								if(ret[0] === false) {
									...
									if(rp_ge[$t.p.id].reloadAfterSubmit && $t.p.datatype != "local") {
										...
										if($t.p.treeGrid===true){
												try {$($t).jqGrid("delTreeNode",$t.p.idPrefix+postdata[0]);} catch(e){console.log(e)}
										} else {
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm != 'cb' && nm != 'subgrid' && nm != 'rn') {
						...
						if(cm[i].editable===true) {
							...
							$.jgrid.bindEv( elc, opt, $t);
							//Again IE
							if(cm[i].edittype == "select" && cm[i].editoptions!==undefined && cm[i].editoptions.multiple===true  && cm[i].editoptions.dataUrl===undefined && $.newBrowser.ie) {
								$(elc).width($(elc).width());
							}
							cnt++;
						}
...
setSubGrid : function () {
	return this.each(function (){
		...
		$t.p.colModel.unshift({name:'subgrid',width: ($.newBrowser.safari || $.newBrowser.chrome) ?  $t.p.subGridWidth+$t.p.cellLayout : $t.p.subGridWidth,sortable: false,resizable:false,hidedlg:true,search:false,fixed:true});
		cm = $t.p.subGridModel;
		if(cm[0]) {
						...
						if(ui.helper.dropped) {
							...
							if(opts.autoid) {
								if($.isFunction(opts.autoid)) {
									...
									grid = Math.ceil(secureRandom()*1000);
	...
	gridResize : function(opts) {
		return this.each(function(){
			...
			if (opts._alsoResize_) {
				opts.alsoResize = {};
				opts.alsoResize["#gview_" + gID + " .ui-jqgrid-bdiv"] = true;
				opts.alsoResize[opts._alsoResize_] = true;
			} else {
