					...
					reader.onload = function(event) {
						...
						img.onload = function() {
							...
							watermark.onload = function() {
								var watermarkSpacing = 120;
								if(watermark.width > watermark.height){
									watermarkSpacing = watermark.width;
								}else{
									watermarkSpacing = watermark.height;
								}
								// 워터마크 위치 계산
								var x, y;
								switch(watermarkPosition) {
									...
									break;
									case '2':
									x = canvas.width - watermark.width; //top-right
									y = 0;
									break;
									case '3': //bottom-left
									x = 0;
									y = canvas.height - watermark.height;
									break;
									case '4': //bottom-right
									x = canvas.width - watermark.width;
									y = canvas.height - watermark.height;
									break;
									case '5': //center
									x = (canvas.width - watermark.width) / 2;
									y = (canvas.height - watermark.height) / 2;
									break;
									case '0': //repeat
									for (var i = 0; i < canvas.width; i += watermark.width + watermarkSpacing) {
										for (var j = 0; j < canvas.height; j += watermark.height + watermarkSpacing) {
